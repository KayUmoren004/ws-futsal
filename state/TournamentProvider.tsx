...
const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case "addNight":
      return {
        ...state,
        nights: [action.payload, ...state.nights],
        currentNightId: action.payload.id,
      };
    case "loading":
...
const findPlayer = (team: Team, playerId: string) =>
...
export const TournamentProvider = ({
      ...
      try {
        ...
        if (raw) {
          const parsed = JSON.parse(raw) as
            | { nights: GameNight[]; globalPlayers?: Player[] }
            | GameNight[];
          const nightsArr = Array.isArray(parsed) ? parsed : parsed.nights;
          const globalPlayers = Array.isArray(parsed)
            ? []
            : parsed.globalPlayers ?? [];
  ...
  const removeTeam = useCallback(
    (teamId: string) => {
      if (!currentNight) return;
      const teams = currentNight.teams.filter((team) => team.id !== teamId);
      const matches = generateRoundRobin(teams);
      updateNight({ ...currentNight, teams, matches });
    },
    [currentNight, updateNight]
      ...
      const teams = currentNight.teams.map((team) => {
        ...
        const incomingColor =
          data.color && data.color !== team.color
            ? uniqueColor(
                data.color,
                currentNight.teams.filter((t) => t.id !== id)
              )
  ...
  const addPlayer = useCallback(
    (teamId: string, name: string) => {
      ...
      const globalPlayers = state.globalPlayers.some(
        (p) => p.name === name.trim()
      )
        ? state.globalPlayers
  ...
  const updateMatchScore = useCallback(
    (matchId: string, homeScore?: number, awayScore?: number) => {
      ...
      const matches: Match[] = currentNight.matches.map((match) => {
        ...
        const status =
          homeScore === undefined || awayScore === undefined || tie
            ? "scheduled"
            : "completed";
        return {
          ...match,
  ...
  const resetNight = useCallback(() => {
    if (!currentNight) return;
    // Reset everything - clear teams and matches
    updateNight({
      ...currentNight,
      teams: [],
      matches: [],
  ...
  const addGlobalPlayer = useCallback(
    (name: string) => {
      ...
      const existing = state.globalPlayers.find(
  ...
  const addGlobalPlayers = useCallback(
    (names: string[]) => {
      const norm = names.map((n) => formatName(n)).filter(Boolean) as string[];
      if (!norm.length) return [];
      const existingSet = new Set(
        state.globalPlayers.map((p) => p.name.toLowerCase())
      );
      ...
      if (newPlayers.length) {
        dispatch({
          type: "setGlobalPlayers",
          payload: [...state.globalPlayers, ...newPlayers],
        });
      }
      return newPlayers;
    },
